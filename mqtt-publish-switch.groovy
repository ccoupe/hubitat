...
metadata {
		definition (name: "Mqtt Publish Switch", namespace: "ccoupe", 
			author: "Cecil Coupe", 
			importURL: "https://raw.githubusercontent.com/ccoupe/hubitat/master/raw/mqtt-publish-switch.groovy") {
		...
		preferences {
			...
			input name: "onPub", type: "text", title: "On payload to send",
          description: "Optional. String to send (on)", required: false, 
          displayDuringSetup: true
			input name: "offPub", type: "text", title: "Off payload to send",
          description: "Optional. String to send (off)", required: false, 
          displayDuringSetup: true
			input (
	    ...
	    input (
	      ...
	      displayDuringSetup: true, defaultValue: 10, description: "Seconds until auto turning off"
	      )
    }
}
...
def on() {
  onStr = 'on'
  if (settings?.onPub) {
    onStr = settings.onPub
  }
  if (logEnable) log.info "publish ${onStr} to ${settings?.topicPub}"
  interfaces.mqtt.publish(settings?.topicPub, onStr, 1, false)
  if (settings?.AutoOff) {

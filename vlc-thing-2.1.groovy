 ...
 *  Modifed by Cecil Coupe, ccoupe@cableone.net
 *
 ...
 *  Version 2.1.0 (02/20/2020) by Cecil Coupe
      Voice selection
      TTS short code: if text starts with [warning] or [alert] then emphasis=strong
 */
...
metadata {
    definition (name:"VLC Thing", namespace:"cjc", author:"Cecil Coupe+") {
        capability "Actuator"
        capability "Switch"
...
private def myTextToSpeech(text) {
    def lpos = text.indexOf("[")
    def rpos = text.indexOf("]")
    if (lpos == 0 && rpos > lpos) {
      tag = text.substring(0, rpos+1)
      text = text.substring(rpos+1)
      //log.debug "tag: ${tag}"
      if (tag.startsWith("[warn")) {
        text = "<emphasis level=\"strong\">" + text + "</emphasis>"
      } else if (tag.startsWith("[alarm")) {
        text = "<emphasis level=\"strong\">" + "Alert! Alert! " + text + "</emphasis>"
      }
    }
